...
def _emit_status(message: str, *, context: Optional[Dict[str, object]] = None) -> None:
    """Log a status message and forward it to registered subscribers."""

    # Avoid writing directly to stdout because some hosting environments (for example
    # IIS with wfastcgi) treat stdout as the HTTP response channel. Excess output can
    # corrupt the response stream and surface as intermittent 500 errors during long
    # crawls. Using structured logging keeps the details available without breaking
